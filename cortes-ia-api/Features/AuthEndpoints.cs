...
public static class AuthEndpoints {
 public static void Map(WebApplication app){
  app.MapPost("/api/v1/auth/register",async(RegisterDto r,Database db,UserManager<User> users,WalletService wallet,IConfiguration c)=>{
   ...
   db.Notifications.Add(new Notification{UserId=u.Id,Dedupe="verify:"+u.Id,Subject="Confirme seu e-mail",Body=url});
  ...
  app.MapPost("/api/v1/auth/forgot",async(EmailDto r,UserManager<User> users,Database db,IConfiguration c)=>{
   var u=await users.FindByEmailAsync(r.Email);if(u!=null){var token=await users.GeneratePasswordResetTokenAsync(u);db.Notifications.Add(new Notification{UserId=u.Id,Dedupe="reset:"+Guid.NewGuid(),Subject="Redefinir senha",Body=(c["PUBLIC_URL"]??"http://localhost:3000")+"/reset?email="+Uri.EscapeDataString(r.Email)+"&token="+Uri.EscapeDataString(token)});await db.SaveChangesAsync();}return Results.Accepted();
  }).RequireRateLimiting("auth");
  app.MapPost("/api/v1/auth/reset",async(TokenDto r,UserManager<User> users)=>{var u=await users.FindByEmailAsync(r.Email);if(u==null||string.IsNullOrEmpty(r.Password)||!(await users.ResetPasswordAsync(u,r.Token,r.Password)).Succeeded)throw new DomainError("INVALID_TOKEN");await users.UpdateSecurityStampAsync(u);return Results.Ok();}).RequireRateLimiting("auth");
  app.MapPost("/api/v1/auth/mfa/enroll",async(HttpContext h,UserManager<User> users)=>{var u=(await users.FindByIdAsync(Api.User(h).ToString()))!;if(u.TwoFactorEnabled)throw new DomainError("MFA_ALREADY_ENABLED",409);var recent=long.TryParse(h.User.FindFirstValue("auth_time"),out var at)&&DateTimeOffset.UtcNow.ToUnixTimeSeconds()-at<300;if(!recent)throw new DomainError("STEP_UP_REQUIRED",403);await users.ResetAuthenticatorKeyAsync(u);var key=await users.GetAuthenticatorKeyAsync(u);return Results.Ok(new{key,uri=$"otpauth://totp/CortesIA:{Uri.EscapeDataString(u.Email!)}?secret={key}&issuer=CortesIA"});}).RequireAuthorization();
