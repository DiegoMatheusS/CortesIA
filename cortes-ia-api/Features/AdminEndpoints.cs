...
public static class AdminEndpoints {
 public static void Map(WebApplication app){var g=app.MapGroup("/api/v1/admin").RequireAuthorization("Admin");
  ...
  g.MapGet("/tickets",async(Database d)=>await d.Tickets.OrderByDescending(x=>x.CreatedAt).Take(100).ToListAsync());
   g.MapPost("/tickets/{id:guid}/reply",async(Guid id,ReplyDto r,HttpContext h,Database d)=>{var t=await d.Tickets.FindAsync(id)??throw new DomainError("NOT_FOUND",404);if(!new[]{"IN_REVIEW","ANSWERED","RESOLVED"}.Contains(r.Status))throw new DomainError("INVALID_STATUS");t.Reply=r.Reply;t.Status=r.Status;Api.Audit(d,Api.User(h),"TICKET_REPLY",id.ToString(),"Resposta registrada");await NotificationEndpoints.Queue(d,t.UserId,$"ticket:{t.Id}:{r.Status}:{Guid.NewGuid()}","SUPPORT_"+r.Status,"SUPPORT",r.Status=="RESOLVED"?"Seu chamado foi resolvido":"Você recebeu uma resposta do suporte",r.Status=="RESOLVED"?"O chamado foi marcado como resolvido. Você pode reabrir caso ainda precise de ajuda.":r.Reply,"DEFAULT_ON");await d.SaveChangesAsync();return Results.Ok();});
