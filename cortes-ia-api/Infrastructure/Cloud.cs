...
public class Cloud {
 ...
 readonly bool publicEndpointUsesHttp;
 public Cloud(IConfiguration c) {
  ...
  if(!string.IsNullOrEmpty(endpoint)){
   sc.ServiceURL=endpoint;
   pc.ServiceURL=c["S3_PUBLIC_ENDPOINT"]??endpoint;
   publicEndpointUsesHttp=pc.ServiceURL.StartsWith("http://",StringComparison.OrdinalIgnoreCase);
   qc.ServiceURL=endpoint;sc.AuthenticationRegion=region;pc.AuthenticationRegion=region;qc.AuthenticationRegion=region;
  }
  S3=new AmazonS3Client(sc);PublicS3=new AmazonS3Client(pc);Sqs=new AmazonSQSClient(qc);
 }
 string PublicUrl(string url)=>publicEndpointUsesHttp&&url.StartsWith("https://",StringComparison.OrdinalIgnoreCase)?"http://"+url[8..]:url;
 public string Download(string key)=>PublicUrl(PublicS3.GetPreSignedURL(new GetPreSignedUrlRequest{BucketName=Bucket,Key=key,Verb=HttpVerb.GET,Expires=DateTime.UtcNow.AddMinutes(5)}));
 public string Part(string key,string uploadId,int part)=>PublicUrl(PublicS3.GetPreSignedURL(new GetPreSignedUrlRequest{BucketName=Bucket,Key=key,Verb=HttpVerb.PUT,UploadId=uploadId,PartNumber=part,Expires=DateTime.UtcNow.AddMinutes(15)}));
