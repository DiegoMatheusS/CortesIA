...
| 6 Editor | Editor short-form, corte manual, timeline por segmentos, dividir/remover trecho, revisão não destrutiva, desfazer/refazer local, edição de texto/sincronismo, presets de legenda, estilos visuais, **crop arrastável/redimensionável diretamente sobre o master**, histórico, restauração de revisão como nova revisão, seleção de capa a partir de frame real do master, preview regenerável/versionado e export por revisão | Edição avançada da capa e quote para operações pesadas adicionais |
