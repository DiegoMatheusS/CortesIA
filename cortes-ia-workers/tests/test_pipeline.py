...
class PipelineTests(unittest.TestCase):
    def test_fixture_pipeline_keeps_full_master_and_preview(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            lease={'stage':'PROCESS','projectId':'test','maxBytes':30_000_000_000,'maxDurationMs':10_800_000,'outputPrefix':'projects/test/jobs/123/1/','payload':{'sourceKey':'quarantine/test/video','config':{'quantity':5,'durationMode':'UP_TO_1_MIN','features':['zoom','blur','cover']},'modality':'trial'}}
            progress=[]
    ...
    def test_editor_preview_renders_saved_revision_plan(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            lease={
                'stage':'PREVIEW','projectId':'test','maxBytes':30_000_000_000,'maxDurationMs':10_800_000,
    ...
    def test_cover_job_extracts_selected_master_frame(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            env={'APP_ENV':'development','MEDIA_EXECUTION':'local','SCAN_MODE':'disabled-development','MEDIA_TASK_ROOT':str(root/'tasks')}
            lease={
                'stage':'COVER','projectId':'test','maxBytes':30_000_000_000,'maxDurationMs':10_800_000,
